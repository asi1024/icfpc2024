...
class Main extends pot_core_App {
		...
		window.document.getElementById("import-mod").onclick = function() {
			return window.navigator.clipboard.readText().then(function(text) {
				...
				while(_g < _g1.length) {
					...
					++_g;
					if(c.data != CellData.None) {
						_gthis.copiedCells.push(c);
					}
				}
				_gthis.normalizeCopiedCells();
				return _gthis.mode = Mode.Paste;
			});
		};
		window.document.getElementById("export-mod").onclick = function() {
			let sel = _gthis.selectedCells(_gthis.w.input);
			if(sel.length == 0) {
				return;
			}
			let f = new Field();
			...
			if(l._hx_index == 1) {
				...
				if(r._hx_index == 1) {
					...
					if(BigIntWrapper.gt(_g,BigIntWrapper.fromInt(-100)) && BigIntWrapper.lt(_g,BigIntWrapper.fromInt(100)) && BigIntWrapper.gt(_g1,BigIntWrapper.fromInt(-100)) && BigIntWrapper.lt(_g1,BigIntWrapper.fromInt(100))) {
						...
						console.log("src/Main.hx:659:",dx + " " + dy);
						return [-dy,-dx];
					}
				}
			}
	...
	moveSelection(f,di,dj) {
		...
		while(_g < _g1.length) {
			let c = _g1[_g];
			++_g;
			let diff = this.warpIJDiff(f,c);
			if(diff == null) {
				continue;
			}
			let dst = f.cellAt(c.i + diff[0],c.j + diff[1]);
			let srcIn = c.selected;
			let dstIn = dst.selected;
			if(!srcIn && dstIn) {
				warpsIn.push(c);
			}
			if(srcIn && !dstIn) {
		...
		while(_g2 < warpsIn.length) {
			...
			if(_g._hx_index == 1) {
				if(_g1._hx_index == 1) {
					l.data = CellData.Int(BigIntWrapper.sub(_g.a,BigIntWrapper.fromInt(dj)));
					r.data = CellData.Int(BigIntWrapper.sub(_g1.a,BigIntWrapper.fromInt(di)));
				} else {
		...
		while(_g3 < warpsOut.length) {
			...
			if(_g._hx_index == 1) {
				if(_g1._hx_index == 1) {
					l.data = CellData.Int(BigIntWrapper.add(_g.a,BigIntWrapper.fromInt(dj)));
					r.data = CellData.Int(BigIntWrapper.add(_g1.a,BigIntWrapper.fromInt(di)));
				} else {
					throw haxe_Exception.thrown("internal error");
				}
			} else {
		...
		let _this = this.selectedCells(f);
		let result = new Array(_this.length);
		let _g4 = 0;
		...
		this.pasteAt(f,result,di,dj);
		let _g6 = 0;
		while(_g6 < result.length) {
			let c = result[_g6];
			++_g6;
	...
	normalizeCopiedCells() {
		...
		let _this = this.copiedCells;
		let _g_current = 0;
		while(_g_current < _this.length) {
			let _g_value = _this[_g_current];
			this.copiedCells[_g_current++] = new Cell(_g_value.i - mi,_g_value.j - mj,_g_value.data);
		}
	}
	processInput() {
		let mouse = this.input.mouse;
		...
		case 3:
			if(mouse.dright == 1 || this.key("Escape")) {
				...
				if(cell != null && mouse.dleft == 1) {
					this.cursorI = cell.i;
					this.cursorJ = cell.j;
					this.pasteAt(f,this.copiedCells,cell.i,cell.j);
					this.mode = Mode.Edit;
					this.clearSelection(f);
					f.cellAt(this.cursorI,this.cursorJ).selected = true;
				}
		...
		case 4:
			...
			if(this.key("Escape") || mouse.dright == 1) {
				_g2.text = "";
				this.mode = Mode.Edit;
			} else if(this.key("e") || this.key("Enter") || mouse.dleft == 1) {
				_g2.text = "";
	...
	drawInfo() {
		...
		case 5:
			...
			if(this.w.result != CellData.None) {
				let text = "Result: " + _$Main_CellDataTools.toString(this.w.result);
				this.g.c2d.fillText(text,0,y);
				y += 18.0;
				y += 9.;
			}
	...
	drawField(f) {
		...
		while(_g < 48) {
			...
			while(_g1 < 48) {
				...
				if(cell.selected) {
					...
					let rgb = ((cell.i ^ cell.j) & 1) == 0 ? 0.95 : 1;
					this.g.fillColorImpl(rgb,rgb,rgb,1);
				}
		...
		if(this.mode == Mode.Paste) {
			...
			while(_g < _g1.length) {
				...
				if(c.selected) {
					...
					let rgb = 1;
					this.g.fillColorImpl(rgb,rgb,rgb,0.5);
				}
				this.g.c2d.fillRect(0,0,1,1);
				let _g2 = this.mode;
				if(_g2._hx_index == 4) {
		...
		if(this.mode == Mode.Edit || this.mode == Mode.Select) {
			let cc = f.cellAt(this.cursorI,this.cursorJ);
			let _g = 0;
			let _g1 = f.cells;
			while(_g < _g1.length) {
				let c = _g1[_g];
					...
					if(l._hx_index == 1) {
						...
						if(r._hx_index == 1) {
							...
							if(BigIntWrapper.gt(_g,BigIntWrapper.fromInt(-100)) && BigIntWrapper.lt(_g,BigIntWrapper.fromInt(100)) && BigIntWrapper.gt(_g1,BigIntWrapper.fromInt(-100)) && BigIntWrapper.lt(_g1,BigIntWrapper.fromInt(100))) {
								...
								this.g.strokeColorImpl(1,0,0,a);
								let en_x = dstJ + 0.5;
								let en_y = dstI + 0.5;
								let x = c.j + 0.5;
								let y = c.i + 0.5;
